...
function buildDocxChildren(data: DevelopmentContractInput) {
  ...
  return [
    ...
    p("Приложение № 1", { pageBreakBefore: true, right: true, indent: false, spacingAfter: 0 }),
    p(`к договору № ${data.number} от ${dateFormatted}`, { right: true, indent: false }),
    title("СПЕЦИФИКАЦИЯ"),
    ...
    p("Приложение № 2", { pageBreakBefore: true, right: true, indent: false, spacingAfter: 0 }),
    p(`к договору № ${data.number} от ${dateFormatted}`, { right: true, indent: false }),
    title("Заявка"),
