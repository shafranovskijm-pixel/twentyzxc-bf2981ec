...
const OrgClientsTab = ({ organizationId }: { organizationId: string }) => {
  ...
  const handleCreate = async () => {
    ...
    setSaving(true);
    try {
      const { error } = await supabase.from("org_clients" as any).insert({
      ...
      queryClient.invalidateQueries({ queryKey: ["org-clients", organizationId] });
      setShowCreate(false);
      setName("");
      setPhone("");
      setEmail("");
  ...
  const deleteClient = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("org_clients" as any).delete().eq("id", id);
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["org-clients", organizationId] });
                ...
                {clients.map((c: any) => (
                  <TableRow key={c.id}>
                    ...
                    <TableCell className="text-right sticky right-0 bg-card">
                      <Button variant="ghost" size="icon" onClick={() => deleteClient.mutate(c.id)} className="text-destructive"><Trash2 className="h-4 w-4" /></Button>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
