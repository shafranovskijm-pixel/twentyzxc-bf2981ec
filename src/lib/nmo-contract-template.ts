...
export function generateNmoContractHtml(data: DocumentData): string {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Договор НМО №${num}</title>${baseStyles}</head><body>
    ...
    <div class="section">
      <p class="indent"><strong>${c.company_short_name || c.company_name}</strong>, в дальнейшем именуемое Исполнитель, ИНН ${c.company_inn}, с одной стороны и <strong>${cl.name}</strong>, именуемое в дальнейшем Заказчик, в лице ${declinePost(cl.director_post || "Директор").toLowerCase()} ${declineFullName(cl.director_name)}, ${getActingPhrase(cl.director_name)} на основании Устава, с другой стороны, вместе именуемые стороны, заключили настоящий Договор о нижеследующем:</p>
    </div>
