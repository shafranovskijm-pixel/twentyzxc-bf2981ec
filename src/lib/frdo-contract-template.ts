...
export function generateFrdoContractHtml(data: DocumentData): string {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Договор ФРДО №${num}</title>${baseStyles}</head><body>
    ...
    <div class="section">
      ...
      <p class="indent">1.1. УЦ оказывает Заказчику услуги по выгрузке данных заказчика о выданных документах об образовании и (или) о квалификации на портал Федеральной информационной системы «Федеральный реестр сведений о документах об образовании и (или) о квалификации, документах об обучении» (далее ФИС ФРДО)${/вайб/i.test(cl.name || "") ? `, размещаемый по адресу <strong>http://10.3.48.20/</strong>` : ""} и обеспечивает информационно-консультационную поддержку Заказчика по работе в ФИС ФРДО (далее – услуги), а Заказчик обязуется принять и оплатить данные услуги.</p>
