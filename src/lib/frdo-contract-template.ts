...
export function generateFrdoContractHtml(data: DocumentData): string {
  ...
  return `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Договор ФРДО №${num}</title>${baseStyles}</head><body>
    ...
    <div class="section">
      <p class="indent"><strong>${c.company_short_name || c.company_name}</strong>, в дальнейшем именуемое УЦ (Учебный центр), ИНН ${c.company_inn}, с одной стороны и ${(() => {
    ...
    <div class="section">
      ...
      <p class="indent">1.1. УЦ оказывает Заказчику услуги по выгрузке данных заказчика о выданных документах об образовании и (или) о квалификации на портал Федеральной информационной системы «Федеральный реестр сведений о документах об образовании и (или) о квалификации, документах об обучении» (далее ФИС ФРДО), размещаемой по адресу ${(() => {
        const isVibe = /вайб/i.test(cl.name || "");
        return isVibe ? `<strong>http://10.3.48.20/</strong>` : `https://frdocheck.obrnadzor.gov.ru/`;
      })()}, и обеспечивает информационно-консультационную поддержку Заказчика по работе в ФИС ФРДО (далее – услуги), а Заказчик обязуется принять и оплатить данные услуги.</p>
